...
impl LoadedPlugin {
    ...
    pub fn load(path: impl AsRef<Path>) -> Result<Self, String> {
        ...
        let init_sym: Symbol<unsafe extern "C" fn() -> *const AerofiPlugin> = 
            unsafe { lib.get(b"aerofi_plugin_init\0") }.map_err(|e| format!("Missing aerofi_plugin_init: {e}"))?;
        ...
        if plugin.api_version != 1 {
            return Err(format!("Unsupported plugin API version: {}", plugin.api_version));
        }
        ...
        let name = if metadata.name.is_null() {
            ...
            unsafe { CStr::from_ptr(metadata.name) }.to_string_lossy().into_owned()
        };
        
        let description = if metadata.description.is_null() {
            String::new()
        } else {
            unsafe { CStr::from_ptr(metadata.description) }.to_string_lossy().into_owned()
        };
        
        let prefix = if metadata.prefix.is_null() {
            String::new()
        } else {
            unsafe { CStr::from_ptr(metadata.prefix) }.to_string_lossy().into_owned()
        };
